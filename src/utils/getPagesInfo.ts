...
function getPagesInfo(pagePath, dir = '') {
  if (dir) {
    // 忽略的页面
    if (config.pagesRegexp.ignore.length > 0 && config.pagesRegexp.ignore.some((item) => !!item.exec(dir.replace(/\\/g, '/')))) {
      return []
    }
    // 指定的页面
    if (config.pagesRegexp.pages.length > 0 && !config.pagesRegexp.pages.some((item) => !!item.exec(dir.replace(/\\/g, '/')))) {
      return []
    }
  }
  ...
  return flatten(
    directories.map((fileName) => {
      ...
      return getPagesInfo(filePath, path.relative(config.paths.appSrc, filePath))
    })
  )
}
